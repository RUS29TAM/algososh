...
describe('Тест для компонента "Связанный список"', () => {
    beforeEach(() => {
        cy.visit('/list')
    });
    ...
    describe('Тестирование алгоритма "Связанный список"', () => {
        ...
        it('Удалить ноду из tail, рендер и визуализация корректно отработаны', () => {
            const tailText = 'tail';
            const headText = 'head'
            cy.contains('Удалить из tail').click();

            cy.get(circleContent).eq(5).siblings().find(circleSmall)
                .should('have.css', 'border', '4px solid rgb(210, 82, 225)')
                .should('not.have.text', '');
            cy.wait(SHORT_DELAY_IN_MS);
